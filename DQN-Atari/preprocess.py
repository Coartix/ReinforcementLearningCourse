...
class CustomWrapper(gym.Wrapper):
    ...
    def __init__(
        ...
        Args:
            ...
            grayscale_obs (bool): if True, then gray scale observation is returned, otherwise, RGB observation
                is returned.
            luminance_obs (bool): if True, then luminance observation is returned, otherwise, RGB observation
        ...
        self.luminance_obs = luminance_obs
        self.grayscale_newaxis = grayscale_newaxis
        self.scale_obs = scale_obs

        # buffer of most recent two observations for max pooling
        assert isinstance(env.observation_space, Box)
        if grayscale_obs and not luminance_obs:
            self.obs_buffer = [
        ...
        _shape = (screen_size, screen_size, 1 if grayscale_obs and not luminance_obs else 3)
        if grayscale_obs and not grayscale_newaxis or luminance_obs:
    ...
    def step(self, action):
        ...
        for t in range(self.frame_skip):
            ...
            if t == self.frame_skip - 2:
                if self.grayscale_obs and not self.luminance_obs:
    ...
    def _rgb_to_yuv(self, rgb_obs):
        r, g, b = rgb_obs[:, :, 0], rgb_obs[:, :, 1], rgb_obs[:, :, 2]
        y = 0.299 * r + 0.587 * g + 0.114 * b
        y = np.asarray(y, dtype=np.uint8)
        # u = (b - y) * 0.565
        # v = (r - y) * 0.713
        y_obs = np.expand_dims(y, axis=-1)
        # Take out last dimension from y_obs
        y_obs = np.squeeze(y_obs, axis=-1)
        return y_obs
    ...
    def _get_obs(self):
        ...
        if self.luminance_obs:
            obs = self._rgb_to_yuv(obs)
        elif self.grayscale_obs and self.grayscale_newaxis:
